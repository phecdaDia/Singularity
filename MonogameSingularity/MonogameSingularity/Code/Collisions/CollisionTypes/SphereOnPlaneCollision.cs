...
namespace Singularity.Core.Collisions.CollisionTypes
{
	...
	{
		public static bool GetCollision(SphereCollision collidableA, PlaneCollision collidableB, out Vector3 position,
			out Vector3 normal, out float scale1, out float scale2)
		{
			var eq = VectorMathHelper.SolveLinearEquation(collidableB.Normal / collidableB.Normal.Length(),
				collidableB.SpanVector1,
		...
		public static Vector3 HandleCollision(SphereCollision collider, PlaneCollision collidable, Vector3 position,
			Vector3 normal)
		{
